            ...
            <div className="w-[20%] relative border border-[#bfbfbf] bg-white mr-[50px]">
                {categories.map((c, i) => <div key={i} className="grid grid-cols-[30px_1fr] cursor-pointer group w-[100%]">
                    ...
                    {subCategories.map((sc, i) => {
                        if (sc.key == c.key) return <div key={i} className="absolute z-50 left-[100%] top-0 min-h-[100%] w-[200px] hidden group-hover:flex flex-col bg-transparent">
                            <div className="relative flex-grow ml-3 bg-white">
                                {sc.category.map((sc1, i) => <div className='group/sub' key={i}>
                                    <span>{sc1}</span>
                                    {subCategories2.map((sc2, i) => {
                                        if (sc1 == sc2.key) return <div className="absolute z-50 left-[100%] top-0 min-h-[100%] w-[200px] hidden group-hover/sub:flex flex-col bg-white" key={i}>
                                            <div className="my-[10px] px-[10px] border-l-2 border-[#bfbfbf] flex-grow flex flex-row flex-wrap justify-between">
